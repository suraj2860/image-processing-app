        ...
        try {
            ...
            if (webhookUrl) {
                await axios.post(webhookUrl, {
                    requestId: request.requestId, // Fixed: use request.requestId instead of requestId
                    status: 'completed',
            ...
            console.error(`Error processing request ${request.requestId}:`, error); // Fixed: use request.requestId and removed reference to job
            ...
            // Don't throw the error here, as we want to continue processing other requests
        }
    }
};
...
export { processImages, setupImageProcessor };

function setupImageProcessor() {
    imageQueue.process(1, async (job) => {
        console.log(`Processing job: ${job.id}`);
        await processImages(); // Remove job parameter as it's not used
    });
